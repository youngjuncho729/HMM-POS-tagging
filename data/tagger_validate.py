...
if __name__ == "__main__":
    ...
    print(
        "Training on training1.txt, running tests on test1.txt. "
        "Output --> output1.txt"
    )
    ...
    with open("output1.txt", "r") as output_file, open(
        "training1.txt", "r"
    ) as solution_file, open("results1.txt", "w") as results_file:
        ...
        for index in range(len(output)):
            if output[index] != solution[index]:
                ...
                if solution[index].split(":")[1].strip() not in miss:
                    miss[solution[index].split(":")[1].strip()] = 0
                miss[solution[index].split(":")[1].strip()] += 1
            else:
                total_matches = total_matches + 1

        accuracy = total_matches / len(output)
        ...
        results_file.write(f"Total accuracy: {accuracy}.\n")
